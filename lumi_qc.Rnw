...
<<auto_outlier, echo=FALSE, cache=TRUE, warning=FALSE, message=FALSE, results='hide'>>=
mean_dist = apply(as.matrix(d), 2, mean)
outliers = which(mean_dist > mean(mean_dist) + 2*sd(mean_dist))
#if (length(outliers) > 0) knit_child("lumi_outliers.Rnw")
@

All of the quality control analysis presented about points to TF3TF4B being an outlier array that should be removed from the analysis, as it is likely to be harmful if left in. This array is removed for all subsequent analyses.
