...
group_factor = as.factor(pData(qc_data)$knockdown)
...
#apply(as.matrix(d), 2, sum) #can use for outlier detection - how to decide what's large?
...
ggdendrogram(h, leaf_labels=F, labels=F, rotate=T) +
  ...
  geom_text(data=labs, aes(label=label, x=x, y=y, colour=as.factor(group)), hjust=1.0, size=1.5)
